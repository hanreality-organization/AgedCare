...
//      initLog();
   ...
   {  if (transp!=null) try {  return Class.forName("org.org.zoolu.sip.provider.ConnectedTransport").isInstance(transp);  } catch (ClassNotFoundException e) {}
      // else
      return false;
   }    
   ...
   {  printLog("adding SipProviderListener in promiscuous mode", Log.LEVEL_MEDIUM);
      ...
      {  printWarning("trying to add_icon an already present SipProviderListener in promiscuous mode.", Log.LEVEL_HIGH);
      }
      else
   ...
   {  printLog("adding a SipProviderExceptionListener", Log.LEVEL_MEDIUM);
      ...
      {  printWarning("trying to add_icon an already present SipProviderExceptionListener.", Log.LEVEL_HIGH);
      }
      else
   ...
   {  // in case it is passed only the 'user' field, add_icon '@'<outbound_proxy>[':'<outbound_port>]
