...
public class IpAddress
{
   ...
   public static IpAddress getLocalHostAddress()
   {
      ...
      {  // continue only with java4 VM (e.g. jdk1.4) or later, otherwise ClassNotFoundException is thrown
         ...
         Enumeration networks=(Enumeration)network_interface_class.getMethod("getNetworkInterfaces",nullclass).invoke(null,nullobject);
         ...
         {  //Enumeration iaddrs=((java.net.NetworkInterface)networks.nextElement()).getInetAddresses(); // this line can be compiled only with java4
            Enumeration iaddrs=(Enumeration)network_interface_class.getMethod("getInetAddresses",nullclass).invoke(networks.nextElement(),nullobject);
            ...
            {  InetAddress iaddr=(InetAddress)iaddrs.nextElement();
               ...
               if (iaddr.getClass().getName().equals("java.net.Inet4Address") && !((Boolean)inet4_address_class.getMethod("isLoopbackAddress",nullclass).invoke(iaddr,nullobject)).booleanValue()) ip_address=new IpAddress(iaddr);
            }
         }
      }
