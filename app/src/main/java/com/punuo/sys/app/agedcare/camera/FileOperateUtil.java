package com.punuo.sys.app.agedcare.camera;
...
public class FileOperateUtil {
    ...
    public static String getFolderPath(Context context, int type, String rootPath) {
        ...
        //添加应用存储路径
        pathBuilder.append(Environment.getExternalStorageDirectory().getAbsolutePath() + "/fanxin/");
        //添加文件总目录
    ...
    public static boolean deleteThumbFile(String thumbPath, Context context) {
        ...
        //源文件路径
        String sourcePath;
        if (thumbPath.contains("video")) {
            sourcePath = thumbPath.replace(context.getString(R.string.Thumbnail),
                    context.getString(R.string.Video));
            sourcePath=sourcePath.replace(".jpg",".mp4");
        } else {
            sourcePath = thumbPath.replace(context.getString(R.string.Thumbnail),
    ...
    public static boolean deleteSourceFile(String sourcePath, Context context) {
        ...
        //缩略图文件路径
        String thumbPath;
        if (sourcePath.contains("video")) {
            thumbPath = sourcePath.replace(context.getString(R.string.Video),
                    context.getString(R.string.Thumbnail));
            thumbPath=thumbPath.replace(".mp4",".jpg");
        } else {
            thumbPath = sourcePath.replace(context.getString(R.string.Image),
                    context.getString(R.string.Thumbnail));
        }
