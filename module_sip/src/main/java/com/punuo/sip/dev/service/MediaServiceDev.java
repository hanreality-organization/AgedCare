...
public class MediaServiceDev extends NormalDevRequestService<MediaData> {
    ...
    protected void onSuccess(Message msg, MediaData result) {
        ...
        onResponse(msg);
        //TODO 启动视频编码
        if (com.punuo.sip.H264Config.monitorType == com.punuo.sip.H264Config.DOUBLE_MONITOR_NEGATIVE) {
            Log.v(TAG, "响应双向视频");
            SipQueryRequest request = new SipQueryRequest(H264ConfigDev.targetDevId);
            SipUserManager.getInstance().addRequest(request);
        } else if (com.punuo.sip.H264Config.monitorType == com.punuo.sip.H264Config.DOUBLE_MONITOR_POSITIVE) {
            Log.v(TAG, "主动双向视频");
            EventBus.getDefault().post(new MonitorEvent(com.punuo.sip.H264Config.DOUBLE_MONITOR_POSITIVE, AccountManager.getTargetDevId()));
        }
