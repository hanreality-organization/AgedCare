package com.punuo.sip.user.message;
...
public class SipUserMessageFactory extends MessageFactory {
    ...
    public static Message createSubscribeRequest(NameAddress to, NameAddress from, String body) {
        Message message = createRequest(SipUserManager.getInstance(), SipMethods.SUBSCRIBE, sipURL,
                to, from, contact, null);
        if (!TextUtils.isEmpty(body)) {
            message.setBody(CONTENT_TYPE, body);
        }
    ...
    public static Message createOptionsRequest(NameAddress to, NameAddress from, String body) {
        Message msg = createRequest(SipUserManager.getInstance(), SipMethods.OPTIONS, sipURL,
                to, from, contact, null);
        if (!TextUtils.isEmpty(body)) {
            msg.setBody(CONTENT_TYPE, body);
        }
    ...
    public static Message createInviteRequest(NameAddress to, NameAddress from, String body) {
        Message msg = createInviteRequest(SipUserManager.getInstance(), sipURL, to, from, contact, null);
        if (!TextUtils.isEmpty(body)) {
            msg.setBody(CONTENT_TYPE, body);
        }
        return msg;
    ...
    public static Message createByeRequest(NameAddress to, NameAddress from) {
        return createRequest(SipUserManager.getInstance(), SipMethods.BYE, sipURL, to, from, contact, null);
    }
