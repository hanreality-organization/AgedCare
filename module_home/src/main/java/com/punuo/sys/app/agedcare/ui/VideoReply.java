...
//        timer.schedule(task, 8000);
    ...
    @OnClick({R2.id.bt_accept, R2.id.bt_refuse,})
        ...
        if (id == R.id.bt_accept) {
            SipInfo.toDev = new NameAddress(devName, sipURL);
            Message request1 = SipMessageFactory.createNotifyRequest(SipInfo.sipUser, SipInfo.toDev,
            ...
            SipInfo.toDev = new NameAddress(devName, sipURL);
            Message request2 = SipMessageFactory.createNotifyRequest(SipInfo.sipUser, SipInfo.toDev,
                    SipInfo.user_from, BodyFactory.createCallReply("refuse"));
            SipInfo.sipUser.sendMessage(request2);
            isanswering = false;
            finish();
